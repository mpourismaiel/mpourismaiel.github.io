...
export default function BlogPostPage({ posts }: { posts: BlogLinkType[] }) {
  return (
    <BlogPostLayout seo={seo} relatedPostsPool={posts}>
      <p>
        We started working on UniDeck in November 2024 as a way to ease how our
        team managed clients and projects. We were using various tools such as
        Trello for task management, Google Calendar and Meet for meetings, Gmail
        for communication, GitHub for code, and Grafana for monitoring.
        Navigating all of these apps was not that complicated, but still, one
        common thing was that going into a meeting, someone would have missed
      ...
      <p>
        Our solution to this problem: UniDeck. A drag-and-drop tool that would
        connect to any tool we'd like and display a widget with important
        information from that service. We started implementing widgets for the
        services we used most frequently, and by the end of 2024, we had a solid
        app that helped our productivity, organization, and kept everyone in the
        loop.
      </p>
      ...
      <p>
        ...
        service requires the user to connect their account from that service and
        specify which workspace from that service should be used. From then on,
        whenever they open UniDeck, they can see all the important information
        from that service, such as any updates to a certain project's pull
        requests or issues on GitHub, or what issues have been created on their
        Jira workspace.
      </p>
      <p>
        UniDeck uses a combination of modern tools and techniques to solve
      ...
      <p>
        ...
        problem. as we became the second product of the day, we decided to move
        our focus to unideck and work on it a lot more, polishing everything,
        adding more features and fixing a lot of bugs. We added advanced widgets
        to allow power users to connect any api we haven't gotten to, especially
        connecting to their own services, and have tables and charts for easy
        use without the need for configuring a bunch of complex services. We
        also laid the groundwork for creating a widget marketplace so that in
        the future, anyone can create a widget and allow other users to use it.
      </p>
      <p>
